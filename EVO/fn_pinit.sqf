...
//////////////////////////////////////
player addaction ["<t color='#CCCC00'>View Distance Settings</t>", CHVD_fnc_openDialog, nil,1,false,true,"","[hq_trigger, player] call BIS_fnc_inTrigger"];
//player addaction ["<t color='#CCCC00'>Select Side Mission</t>","[] spawn EVO_fnc_osm;",nil,1,false,true,"","[hq_trigger, player] call BIS_fnc_inTrigger && currentSideMission == 'none'"];
player addaction ["<t color='#CCCC00'>Recruit Infantry</t>","bon_recruit_units\open_dialog.sqf",nil,1,false,true,"","[hq_trigger, player] call BIS_fnc_inTrigger && ((leader group player) == player)"];
player addaction ["<t color='#CCCC00'>HALO Drop</t>", EVO_fnc_paraInsert, nil,1,false,true,"","[hq_trigger, player] call BIS_fnc_inTrigger"];
player addaction ["<t color='#CCCC00'>Group Management</t>","disableserialization; ([] call BIS_fnc_displayMission) createDisplay 'RscDisplayDynamicGroups'",nil,1,false,true,"","[hq_trigger, player] call BIS_fnc_inTrigger"];
...
//Add uav_west Feed
//////////////////////////////////////
uav_westBoard setObjectTexture [0, "#(argb,512,512,1)r2t(uav_westrtt,1)"];
uav_westBoard2 setObjectTexture [0, "#(argb,512,512,1)r2t(uav_westrtt,1)"];
uav_westtv1 setObjectTexture [0, "#(argb,512,512,1)r2t(uav_westrtt,1)"];
uav_westtv2 setObjectTexture [0, "#(argb,512,512,1)r2t(uav_westrtt,1)"];
uav_westtv3 setObjectTexture [0, "#(argb,512,512,1)r2t(uav_westrtt,1)"];

cam = "camera" camCreate [0,0,0];
cam cameraEffect ["Internal", "Back", "uav_westrtt"];
cam attachTo [uav_west, [0,0,0], "PiP0_pos"];
cam camSetFov 0.1;
"uav_westrtt" setPiPEffect [0];
addMissionEventHandler ["Draw3D", {
    _dir = 
        (uav_west selectionPosition "PiP0_pos") 
            vectorFromTo 
        (uav_west selectionPosition "PiP0_dir");
