[{
	...
	//server
		...
		_tskDisplayName = format ["Attack CSAT Installation"];
	...
	if (!isDedicated) then {
	//client
		["TaskAssigned",["","Attack CSAT Installation"]] call BIS_fnc_showNotification;
		CROSSROADS sideChat "All units be advised, forward scouts report CSAT activity at this location. Capture the military installation!";
		handle = [] spawn {
			...
			sleep (random 15);
			CROSSROADS sideChat "Forward scouts report CSAT activity at the military installation is declining. Nice job men!";
