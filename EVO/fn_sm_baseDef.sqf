[{
	...
	if (!isDedicated) then {
		...
		CROSSROADS sideChat "All units be advised, we have CSAT units closing in on the staging base! All available assets move to engage!";
		...
		handle = [] spawn {
			...
			if (player distance spawnBuilding < 1000) then {
				...
				["PointsAdded",["NATO completed a sidemission.", 10]] call BIS_fnc_showNotification;
			};
			sleep (random 15);
			CROSSROADS sideChat "The CSAT counter attack has been defeated. Get back out there!";
			["TaskSucceeded",["","CSAT Counterattack Defeated"]] call BIS_fnc_showNotification;
			currentSideMission = "none";
			publicVariable "currentSideMission";
