...
[{
	...
	//client
		...
		CROSSROADS sideChat "All units be advised, forward scouts report CSAT convoy activity. Check the map and ambush their supply route!";
		handle = [] spawn {
			...
			if (currentSideMissionStatus == "success") then {
				...
				sleep (random 15);
				["TaskSucceeded",["","CSAT Convoy Destroyed"]] call BIS_fnc_showNotification;
				CROSSROADS sideChat "Forward scouts report the convoy is retreating, nice job men!";
			} else {
				["TaskFailed",["","CSAT Convoy Escaped"]] call BIS_fnc_showNotification;
