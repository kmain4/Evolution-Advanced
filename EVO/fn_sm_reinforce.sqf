...
[{
	...
	if (!isDedicated) then {
	//client
		CROSSROADS sideChat "All units be advised, CSAT ground assets are moving to engage our recon elements. All available teams move to reinforce them!";
		...
		handle = [] spawn {
			...
			if (currentSideMissionStatus == "success") then {
				...
				sleep (random 15);
				CROSSROADS sideChat "The CSAT advance on our recon element has been defeated. Nice job men!";
